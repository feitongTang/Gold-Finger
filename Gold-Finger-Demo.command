...
readonly SERVICE_IDENTITY="gold-finger:${PROJECT_ID}:demo"
...
if curl --silent "$APP_URL" >/dev/null 2>&1; then
  fail "端口 3001 已被其他服务占用。请先关闭该服务，再重新启动 Gold-Finger Demo。"
fi
...
GOLD_FINGER_PROJECT_ID="$PROJECT_ID" GOLD_FINGER_MODE="demo" npm run dev:demo
