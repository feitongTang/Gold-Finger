...
仓库当前已实现 MVP 功能闭环：首次资产录入、月度更新、复盘结果、固定投资分类汇总和历史趋势。2026-08-26 深度体验复验发现的数据正确性、录入闭环与本地数据恢复问题已由 Task 14～17 修复，完整 MVP 仍需按 Task 5 重新验收。当前 MVP 仅面向桌面端，手机端适配不在范围内。详细信息见[产品范围](docs/product-scope.md)、[MVP Backlog](docs/mvp-backlog.md)、[人工验收记录](docs/mvp-acceptance.md)和[深度体验测评](audit/product-review-2026-08-26/report.md)。
...
页面底部的“本地数据安全”区域提供推荐的完整备份路径：

1. 选择“导出全部数据”，下载包含全部月份的版本化 JSON 文件。
2. 把文件保存到项目目录之外的安全位置；界面会在当前浏览器中显示最近一次成功导出时间。
3. 需要恢复时选择该 JSON 文件，再选择“恢复备份”。
4. 阅读“将永久替换当前全部月度记录”的提示并明确确认。

应用会先校验备份版本、月份、金额、固定分类和基金净投入合计，再在单个数据库事务中替换数据。文件无效或写入失败时，当前数据不会改变。恢复完成后，先检查已有月份和金额是否符合预期。导出的 JSON 可能包含真实财务数据，不要提交到 Git、Issue 或 Pull Request。
